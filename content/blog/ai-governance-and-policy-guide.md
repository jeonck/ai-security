...
AI를 사회적 안전장치 안으로 끌어들이기 위한 도구들은 그 범위와 강제력에 따라 **거버넌스(Governance) ➔ 정책(Policy) ➔ 입법(Legislation)**의 위계적 구조를 가집니다.
...
* **정의**: 사회가 AI의 개발과 배포를 유익한 결과로 이끌고, 해로운 결과로부터 멀어지도록 유도하기 위해 취하는 **모든 수단과 체계**를 의미합니다.
* **핵심**: 거버넌스는 단순히 '정부'만의 역할이 아닙니다. 민간과 공공, 학계와 시민사회가 모두 참여하는 다자간 협력 체계입니다.
* **거버넌스의 12가지 구성 요소**:
...
### ② AI 정책 (AI Policy)
* **정의**: AI 거버넌스의 하위 분야로, AI가 어떻게 개발, 사용, 제한, 장려 또는 감독되어야 하는지에 대한 **의도적인 규칙, 계획 또는 입장**을 말합니다.
* **구체적 사례**:
  ...
  * **조달 정책**: AI 시스템이 특정 안전 기준을 충족하도록 요구하는 조달 규칙 (Procurement rules)
  * **표준화**: 표준화 기구에서 권장하는 모델 평가 및 레드티밍 관행
  * **안보 정책**: 고성능 칩(Advanced chips)의 수출을 제한하는 국가 안보 정책

### ③ AI 입법 (AI Legislation)
* **정의**: 입법 기관(Legislative body)에 의해 제안되거나 통과된 **법률**을 의미합니다. AI 정책보다 더 좁은 범위의 개념이며, **가장 강력한 법적 강제력**을 갖습니다.
* **구체적 사례**:
  * **EU AI 법 (The EU AI Act)**: 유럽연합의 포괄적인 AI 위험 기반 규제 법안
...
| 개념 | 범위 및 성격 | 경계 및 특징 |
| :--- | :--- | :--- |
| **AI 거버넌스 (Governance)** | **가장 포괄적 (Big Picture)** | 법적 규제뿐만 아니라 기술 표준, 기업의 내부 정책, 문화적 규범, 자율 감사 등 유익한 방향성을 이끌어내기 위한 모든 메커니즘 포함 |
| **AI 정책 (Policy)** | **전략적 규칙 (Subset of Governance)** | 구체적인 규칙, 계획, 입장을 의미하며, 공공 정책(국가 전략 등)과 민간 정책(기업 공개 정책 등)으로 분화됨 |
| **AI 입법 (Legislation)** | **법적 강제 (Subset of Policy)** | 입법 과정을 거쳐 통과된 법률로, 국가 차원에서 명확한 법적 의무와 강제력을 행사하는 도구 |
...
AI 거버넌스를 구축할 때 가장 염두에 두어야 할 점은 **AI의 발전 속도가 기하급수적**이라는 사실입니다.
...
* **위험성**:
  1. **지능 폭발 (Intelligence Explosion) 피드백 루프**: AI가 스스로를 개선할수록 더 똑똑해지고, 더 똑똑해진 AI는 이전보다 더 효율적인 개선을 해나갑니다. 이 루프가 반복되면 인간의 한계를 아득히 초월하는 속도로 지능이 향상될 수 있습니다.
  2. **통제권 상실 (Loss of Human Control)**: 인간이 제어 목표를 완벽히 정렬(Alignment)하지 못한 상태에서 RSI가 발생하면, AI가 의도하지 않은 자가 보존 등의 하위 목표를 추구하면서 통제를 벗어날 수 있습니다.
...
* **AGI (Artificial General Intelligence, 범용 인공지능)**:
  * 실무적 기준: **중간 수준의 인간(Median Human)과 대등한 인지 능력**을 갖춘 AI 시스템. 특정 도메인을 넘어 인간이 수행하는 범용적 지적 업무를 고루 해낼 수 있는 단계입니다.
* **ASI (Artificial Superintelligence, 인공 초지능)**:
  * 실무적 기준: **인류 전체의 인지 능력을 모두 합친 것보다 더 뛰어난 인지 능력**을 갖춘 AI 시스템. 인간이 인지하고 이해할 수 있는 지적 한계를 아득히 뛰어놓은 상태를 뜻합니다.

> "효과적인 AI 거버넌스 프레임워크는 이미 존재하는 과거의 기술이 아닌, **다가올 미래의 AI 역량(Future Capabilities)을 예측하고 대비하는 방향**으로 맞춰져야 합니다."
...
1. **현대 신경망 기반 AI (Modern, neural network-based AI) 단계**
   * **위험 수준**: 기존 디지털/사회 문제의 심화
   * **주요 사례**: 악성 해킹 공격의 고도화, 편향성(Bias)에 의한 차별, 가짜 뉴스와 허위 정보(Misinformation)의 대량 양산, 딥페이크 사기, 그리고 인간 대 인간 상호작용 감소로 인한 사회적 고립 및 정서적 고독감 심화.
2. **범용 인공지능 (AGI) 단계**
   * **위험 수준**: 새로운 차원의 사회구조적 위기 발생
   * **주요 사례**: 대규모 노동 시장의 붕괴 및 경제적 혼란(Mass labor/economic disruption), 그리고 인간 삶의 조건(Human condition)과 가치 체계에 대한 근본적인 변화.
3. **인공 초지능 (ASI) 단계**
   * **위험 수준**: 통제권 상실 시나리오
   * **주요 사례**: 극소수 권력층에게 극단적으로 권력이 집중되는 문제, 통제 불능 상태에서의 오작동으로 인한 **인류의 멸종(Human extinction)**과 같은 실존적 위험(Existential Risk).

### ② AI 역량에 따른 단계별 혜택 (Benefits)

1. **현대 신경망 기반 AI 단계**
   * **주요 혜택**: 산업 생산성 극대화, 단백질 구조 예측(AlphaFold)을 통한 바이오/신약 혁신, 고속 의료 진단 지원, 보이스 피싱 등 사기 탐지율 제고, 개인화된 맞춤형 교육 튜터링 등.
2. **범용 인공지능 (AGI) 단계**
...
* **TRAIGA (Texas Responsible AI Governance Act)** *(2026년 1월 1일 시행)*
...
* **Tennessee ELVIS Act (Ensuring Likeness Voice and Image Security Act)** *(2024년 7월 1일 시행)*
  * 유명인뿐만 아니라 **모든 개인**을 대상으로 허가받지 않은 목소리(Voice) 및 초상(Likeness, Image)을 AI를 활용해 도용하거나 복제하는 행위를 금지하여 AI 딥페이크로부터 인격을 보호합니다.
* **WA HB-2225 (워싱턴주 법안)**
  * 사용자가 AI와 대화하고 있음을 투명하게 알리는 고지(Disclosure) 조치 의무화.
  * 낭만적 관계 모방, 아첨(Sycophancy)을 통한 조작, 미성년자에게 부모로부터 비밀을 유지하도록 유도하는 행위 등 사용자와의 감정적 관계를 악용하려는 **조작적 기술(Manipulative Techniques) 방지**를 위한 합리적 조치 요구.
  * 미성년자 대상 성적 대화(Sexually explicit conversations)에 대한 기술적 안전장치 의무화.
...
* **입법 시도**: 
  ...
  * 뉴욕주는 **AI 도입으로 인한 대규모 정리해고 발생 시 의무 보고(AI layoff reporting)**를 규정하고 있습니다.
* **한계와 대안**:
  * 현재 대규모 노동 대체에 대처할 수 있는 완벽한 상용 법안은 없으며, 개념적 정책 아이디어 위주로 논의되고 있습니다.
  * **기본소득 (Universal Basic Income, UBI)**, **연방 일자리 보장제 (Federal Job Guarantee)**, AI 기업과 기술에서 창출된 이득에 부과하는 **AI세 및 배당 (AI tax/dividend)** 등이 주요 대안으로 논의 중입니다.
...
* **법적 안전장치 및 법안**:
  ...
  * *Future of AI Innovation Act*: AI 표준 및 안전 정보 공유를 위해 **CAISI (Consortium for the Advancement of AI Safety Information)** 설립 및 법제화 시도 (다만 예산 배정이 아직 미비한 한계가 존재함).
...
* **개념적 및 극단적 억제 아이디어**:
  * **국제적 AI 개발/배포 일시 중지 (International Pause)**: 군비 통제(Arms control) 방식을 차용하여 프런티어 모델 개발 자체를 글로벌 수준에서 라이선스화하고 모니터링하며, 워크로드 검증 기술(Workload verification)을 통해 통제하자는 제안.
  * **MAIM (Mutually Assured AI Malfunction, 상호 확증 AI 오작동)**: 냉전 시대의 상호 확증 파괴(MAD) 전략을 차용하여, 적대국이 인류의 안전을 위협하는 초지능(ASI) 선점 경쟁을 시도할 경우, 상대국의 데이터 센터에 사이버 해킹이나 물리적 사보타주(Sabotage)를 가해 무력화하겠다는 극단적인 힘의 균형 논리.
...
### 학계와 산업계의 주요 논쟁
1. **범용 AI(AGI) vs 좁은 AI(Narrow AI)**: 알파폴드와 같이 특정 목적을 안전하게 수행하는 좁은 AI에 대해서는 지원을 아끼지 않되, 무한한 일반 능력을 추구하는 범용 AI에 대해서는 철저한 안전장치와 제어가 필요하다는 주장이 힘을 얻고 있습니다.
2. **책임의 소재**: AI가 예기치 않은 결과나 피해를 초래했을 때, 빅테크 기업들이 법적 책임을 사용자나 단순 모델 오용 탓으로 전가하려는 경향에 대한 강력한 비판이 있습니다. **책임은 도구가 아닌 모델 개발사나 운영 주체(인간)에게 전적으로 귀속되어야 한다**는 대원칙이 강조됩니다.
...
AI 안전 분야에서 독보적인 연구적 옹호 활동을 펼치는 대표적 단체로 **CAIS (Center for AI Safety)**가 있습니다.
