...
### ⑤ 너무 많은 손님이 몰려올 때 (429 Too Many Requests)
문지기가 손님을 막아서는 가장 대표적인 응답이 바로 <strong>`429 Too Many Requests`</strong> 오류입니다. 1초에 100명만 받을 수 있는 식당에 200명이 한꺼번에 들이닥쳤을 때, 가게가 "잠시만 기다려 주세요, 지금은 손님이 너무 많습니다"라고 정중히 돌려보내는 상황과 같습니다.

* **왜 발생하는가**: API Gateway의 스테이지(Stage)나 메서드에 설정된 <strong>사용량 계획(Usage Plan)</strong> 또는 계정 수준의 제한을 넘어서는 초당 요청 수(RPS)가 들어왔을 때 발생합니다. 특정 클라이언트의 과도한 호출이 전체 시스템을 다운시키는 것을 막기 위한 일종의 가드레일(보호 장치)이며, 문지기가 안방 서버에 부담을 주지 않으려고 스스로 거절하는 것이지 안방 서버 자체에 문제가 생긴 것은 아닙니다.
* **`502 Bad Gateway`와의 차이**: 429는 <strong>문지기가 스스로 요청을 거절</strong>한 상황이고, 502는 문지기는 정상인데 <strong>안방(백엔드)이 응답을 제대로 주지 못한</strong> 상황입니다. 전자는 "너무 바빠서 못 받겠다"는 거절 신호이고, 후자는 "안에서 문제가 생겼다"는 장애 신호입니다.
* **해결 전략**: <strong>사용량 계획 조정</strong>(정당한 사용자라면 API 키별 한도를 합리적으로 상향), <strong>지수 백오프와 지터(Exponential Backoff & Jitter)</strong>(클라이언트가 짧은 간격으로 계속 재시도하지 않고 대기 시간을 점진적으로 늘려가며 재요청), <strong>캐싱(Caching) 활용</strong>(동일한 요청이 안방 서버까지 가지 않도록 문지기 단계에서 결과를 캐싱)을 함께 적용합니다.
...
보안은 결코 복잡하고 거창한 시스템이 아닙니다. <strong>"누가 내 서버에 직접 닿을 수 있는가?"</strong>, <strong>"그 방문자가 들고 온 패킷은 안전한가?"</strong>를 명확하게 통제하는 사소한 원칙에서 출발합니다. 
