...
| **기능 목적 (Purpose)** | 이 AI 기능이 무엇을 하는지, 왜 도입했는지 | [NIST AI RMF - Map](../nist-ai-rmf/) |
| **사용자 (Users)** | 누가 사용하는가 (내부 직원 / 일반 고객 / 둘 다) | [NIST AI RMF - Map](../nist-ai-rmf/), [EU AI Act](../eu-ai-act/) (분류 기준) |
| **입력 데이터 (Input Data)** | 모델에 들어가는 데이터의 종류와 출처 (사용자 입력, RAG 문서, 외부 API 등) | [OWASP - 프롬프트 계층](../owasp-llm-top10/) |
| **모델 공급자 (Model Provider)** | 어떤 모델/공급자를 사용하는가 (자체 호스팅 / 외부 API / 오픈소스) | [공급망 리스크](../../infrastructure/supply-chain-risk/), [EU AI Act - GPAI](../eu-ai-act/) |
| **툴/권한 (Tool & Permission Scope)** | 이 AI(에이전트)가 호출할 수 있는 도구와 그 권한 범위 | [OWASP - 권한/툴 실행 계층](../owasp-llm-top10/) |
| **실패 시 영향 (Impact if Compromised)** | 이 기능이 오작동하거나 공격당했을 때 발생하는 비즈니스/안전 영향 | [MITRE ATLAS - Impact](../mitre-atlas/), [EU AI Act - 위험 등급](../eu-ai-act/) |
| **위험 등급 (Risk Classification)** | OWASP 항목 매핑 + EU AI Act 위험 등급 + 자체 우선순위(상/중/하) | [OWASP](../owasp-llm-top10/), [EU AI Act](../eu-ai-act/) |
| **현재 대응 통제 (Current Controls)** | 이미 적용된 기술적/운영적 통제 | [NIST AI RMF - Manage](../nist-ai-rmf/) |
| **잔여 위험 및 추가 통제 (Residual Risk & Next Steps)** | 현재 통제로 충분하지 않은 부분과 향후 계획 | [NIST AI RMF - Measure/Manage](../nist-ai-rmf/) |
| **운영 중지 조건 (Deactivation Trigger)** | 어떤 조건에서 이 기능을 즉시 중단하는가, 중단 권한자는 누구인가 | [NIST AI RMF - Manage](../nist-ai-rmf/) |
...
| **잔여 위험 및 추가 통제** | 시스템 프롬프트 지시만으로는 [프롬프트 인젝션](../../attacks/prompt-injection/) 방어에 충분하지 않음. 출력 구조화(JSON 스키마 강제) 및 별도 검증 레이어 추가 필요. 주문조회 연동 전 [LLM 레드티밍](../../red-teaming/llm-red-teaming/) 수행 필수 |
...
3. **MITRE ATLAS 매핑을 추가**해 보세요. [MITRE ATLAS](../mitre-atlas/) 페이지의 4-레이어 모델(Environment / AI Platform / AI Model / AI Data)을 사용해, "실패 시 영향" 항목에 적은 시나리오가 어느 레이어/전술에 해당하는지 한 줄씩 덧붙입니다.
4. **EU AI Act 등급을 판단**해 보세요. [EU AI Act](../eu-ai-act/) 페이지의 4단계 분류(금지/고위험/제한적 위험/최소 위험) 중 어디에 해당하는지, 그 근거를 한 문단으로 적습니다.
...
다음 단계로, 실제 시스템에 대해 레드팀 관점의 시나리오를 검증해보고 싶다면 [레드팀·포트폴리오 프로젝트](../../red-teaming/portfolio-projects/) 페이지에서 이 위험 등록부를 더 큰 포트폴리오 프로젝트로 확장하는 방법을 확인하세요.
