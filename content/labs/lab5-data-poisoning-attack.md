...
- **공격 시점의 구분**: "추론 시점 공격(적대적 예제)"과 "학습 시점 공격(데이터 포이즈닝)"의 차이를 코드로 직접 구현하며 명확히 구분하게 됩니다.
- **백도어의 은밀함 체감**: label-flipping 비율에 따른 정확도 하락과 백도어 트리거의 ASR을 수치로 측정함으로써, 정상 테스트셋만으로는 오염을 탐지하기 어렵다는 포이즈닝 공격의 위험성을 체감합니다.
- **선제적 방어 관점 확립**: 데이터 검증·이상치 분석·출처(Provenance) 관리가 왜 모델 자체의 견고성 강화보다 먼저 다뤄야 하는 문제인지 설명할 수 있게 됩니다.
