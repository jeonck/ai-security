...
- pip-audit, SBOM, 체크섬, 모델 카드라는 4가지 산출물을 직접 만들어봄으로써, "이 모델이 무엇으로 구성되고 어디서 왔는가"에 답할 수 있는 최소한의 AI 공급망 보안 체계를 경험한다.
- 의존성 취약점 점검 결과가 갖는 "시점성"(현재까지 알려진 것일 뿐, 영구적 안전을 보장하지 않음)이라는 한계를 이해하고, 지속적인 재점검의 필요성을 인식한다.
- [Lab 4](../lab4-model-serving-security/)의 서빙 보안과 이 실습의 공급망 보안을 연결해, 모델의 전체 생애주기(개발 → 배포 → 운영) 관점에서 보안을 바라보는 시각을 갖춘다.
